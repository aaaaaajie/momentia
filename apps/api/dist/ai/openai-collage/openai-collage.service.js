...
let OpenAiCollageService = class OpenAiCollageService {
    ...
    async openaiImageB64(params) {
        ...
        const text = await res.text();
        if (!res.ok)
            throw new Error(`OpenAI images error: ${res.status} ${res.statusText} (model=${model}, size=${normalizedSize}) - ${text}`);
        if (!text.trim().startsWith('{')) {
            throw new Error(`OpenAI images: expected JSON response but got non-JSON body (status ${res.status}). Body head: ${text
                .slice(0, 200)
                .replace(/\s+/g, ' ')}`);
        }
        const json = JSON.parse(text);
        ...
        if (typeof url === 'string' && url.startsWith('http')) {
            ...
            if (!imgRes.ok) {
                throw new Error(`OpenAI images download error: ${imgRes.status} ${imgRes.statusText} - ${Buffer.from(arr)
                    .toString('utf8')
                    .slice(0, 200)}`);
            }
